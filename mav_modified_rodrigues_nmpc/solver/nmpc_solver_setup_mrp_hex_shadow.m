...
eq0= ( 1-(u_mrp(1)^2 + u_mrp(2)^2 + u_mrp(3)^2) )/(1 + u_mrp(1)^2 + u_mrp(2)^2 + u_mrp(3)^2 );
eq13= (2/(1 + u_mrp(1)^2 + u_mrp(2)^2 + u_mrp(3)^2 )) * u_mrp;
...
%err_thrust_desired_frame=[0;rotate_quat([eq0;eq13],[0;F_M(1:3)])]-[0; 0; 0; u_ss_total];   
err_thrust_desired_frame=[0;rotate_quat([eq0;eq13],[0;0;0;F_M(3)])]-[0; 0; 0; u_ss_total];  
err_thrust_inertial_frame=rotate_quat(q_ID,err_thrust_desired_frame);
...
% err_thrust_desired_frame=R_DB * [0;0;f1+f2+f3+f4]-[0; 0; u_ss_total];
%err_thrust_inertial_frame=R_ID*err_thrust_desired_frame;


mrp_dynamics= 0.25*((1-u_mrp'*u_mrp)*eye(3) + 2*S + 2*u_mrp*u_mrp')*e_w;
...
f = dot([e_p; e_v; e_mrp; e_w]) == ...
    [e_v;...
    (1/m)* err_thrust_inertial_frame + external_forces; ...
    mrp_dynamics;... 
